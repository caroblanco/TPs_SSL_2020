...
%type <texto> declarador_directo
%type <texto> decla
%type <texto> declarador
%type <texto> especificadores_declaracion
...
exp_OR_log:   exp_AND_log
            | exp_OR_log OR exp_AND_log
;

exp_AND_log:   exp_OR_in
             | exp_AND_log AND exp_OR_in
;
...
exp_igualdad:   exp_relacional
              | exp_igualdad IGUAL    exp_relacional  
              | exp_igualdad DISTINTO exp_relacional  
;

exp_relacional:   exp_desp
                | exp_relacional      '<'       exp_desp  
                | exp_relacional      '>'       exp_desp  
                | exp_relacional MENOR_IGUAL exp_desp  
                | exp_relacional MAYOR_IGUAL exp_desp  
;

exp_desp:   exp_aditiva
          | exp_desp BIT_SHIFT_L exp_aditiva  
          | exp_desp BIT_SHIFT_R exp_aditiva  
;
...
exp_sufijo:   exp_primaria
            ...
            | exp_sufijo FLECHITA IDENTIFICADOR       
            | exp_sufijo P_INC                   
            | exp_sufijo P_DEC                   
;
...
especificadores_declaracion:   CLASE_ALM           especificadores_declaracion_opc  {$<texto>$ = strdup($<texto>1);}
                             | especificador_tipo  especificadores_declaracion_opc  {$<texto>$ = strdup($<texto>1);}
                             | CONST               especificadores_declaracion_opc  {$<texto>$ = strdup($<texto>1);}
                             | VOLATILE            especificadores_declaracion_opc  {$<texto>$ = strdup($<texto>1);}
;
...
lista_declaradores:   declarador                        {intentarAgregarVar($<texto>1,tempVar,yylineno);}          //fprintf(yyout, "Se declara la variable: \'%s\' de tipo: \'%s\'\n", $<texto>1, tempVar);}          
                    | declarador ',' lista_declaradores {intentarAgregarVar($<texto>1,tempVar,yylineno);}
                                                       
;

declarador:   decla                   {$<texto>$ = strdup($<texto>1);}
            | decla '=' inicializador {$<texto>$ = strdup($<texto>1);}
;
...
especificador_tipo:   tipoDato                   {tempVar = strdup($<texto>1);}              
...
lista_calificadores:   especificador_tipo lista_calificadores_opc
                     | califTipo         lista_calificadores_opc
;
...
decla: puntero_opc declarador_directo {$<texto>$ = strdup($<texto>2);}
;
...
lista_calificadores_tipos:   califTipo
                           | lista_calificadores_tipos califTipo
;

declarador_directo:   IDENTIFICADOR                                                    {$<texto>$ = strdup($<texto>1);}
                    | '(' decla ')'                                                    {$<texto>$ = strdup($<texto>2);}
                    | declarador_directo '[' exp_constante_opc ']'                     {$<texto>$ = strdup($<texto>1);}
                    | declarador_directo '(' lista_tipos_param_opc ')'                 {$<texto>$ = strdup($<texto>1);}
                    | declarador_directo '(' lista_identificadores_opc ')'             {$<texto>$ = strdup($<texto>1);}
;
...
sentencia:   sentencia_exp
           ...
           | sentencia_salto
           | especificadores_declaracion decla sentencia_compuesta  {/*NOBORRARLOESCRIBIOLEAN agregarfn*/}
;
...
sentencia_salto:  CONTINUE ';'                                                                  {fprintf(yyout, "Se encontro la sentencia CONTINUE\n");}
                ...
                | GOTO IDENTIFICADOR ';'                                                        {fprintf(yyout, "Se encontro la sentencia GO TO\n");}               
;  
...
especificadores_declaracion:   claseAlm                 especificadores_declaracion_opc  {tipoVarTemp = strdup($<texto>1); printf("tipo %s\n",tipoVarTemp);}
                             | especificador_tipo       especificadores_declaracion_opc  
                             | califTipo                especificadores_declaracion_opc  
;
...
especificador_tipo:   tipoDato                                 
                    | especificador_struct_union
                    | especificador_enum 
;

tipoDato: CHAR          {tipoVarTemp = strdup($<texto>1);}
        | INT           {tipoVarTemp = strdup($<texto>1);}
        | FLOAT         {tipoVarTemp = strdup($<texto>1);}
        | LONG          {tipoVarTemp = strdup($<texto>1);}
        | SHORT         {tipoVarTemp = strdup($<texto>1);}
        | SIGNED        {tipoVarTemp = strdup($<texto>1);}
        | UNSIGNED      {tipoVarTemp = strdup($<texto>1);}
        | VOID          {tipoVarTemp = strdup($<texto>1);}
        | ENUM          {tipoVarTemp = strdup($<texto>1);}
        | DOUBLE        {tipoVarTemp = strdup($<texto>1);}
;
...
declaracion_parametro:   especificadores_declaracion decla                      {printf("decla: %s\n",$<texto>2);}
                       | especificadores_declaracion declarador_abstracto_opc
;
...
sentencia:   
           especificadores_declaracion decla sentencia_compuesta  {agregarFuncion($<texto>2, NULL);fprintf(yyout, "Se declara la funcion: \'%s\' que devuelve: \'%s\'\n", $<texto>2, $<texto>1);}
           | sentencia_exp
           | sentencia_compuesta
           | sentencia_seleccion
           | sentencia_iteracion
